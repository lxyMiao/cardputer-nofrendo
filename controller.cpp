...
#elif defined(HW_CONTROLLER_M5CARDPUTER)
  ...
  //{
    if (M5Cardputer.Keyboard.isChange())
    {
      //Serial.println("flush key");
       ...
       if(M5Cardputer.Keyboard.isKeyPressed(' ')){ // select
        value ^= (1 << 4);
       }
       if(M5Cardputer.Keyboard.isKeyPressed(KEY_ENTER)){ // enter -> start
        value ^= (1 << 5);
       }
       if(M5Cardputer.Keyboard.isKeyPressed('k')){ // A
        value ^= (1 << 6);
       }
       if(M5Cardputer.Keyboard.isKeyPressed( 'l')){// B
        value ^= (1 << 7);
       }
      /*
